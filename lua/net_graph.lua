    ...
    if updates > updates_gap then
		for i in pairs(graph_settings) do
			...
			if (graph_settings[i].draw_me==nil or conky_parse(tostring(graph_settings[i].draw_me)) == "1") then
 		    ...
 		    if conky_parse("${if_existing /proc/net/route eth0}1${else}0${endif}") then
				graph_settings[i].arg = "eth0"
				draw_graph(graph_settings[i])
		    elseif conky_parse("${if_existing /proc/net/route wlan0}1${else}0${endif}") then
				graph_settings[i].arg = "wlan0"
				draw_graph(graph_settings[i])
    		    else
